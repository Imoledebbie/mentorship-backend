"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
const express_1 = __importDefault(require("express"));
const express_validator_1 = require("express-validator");
const Session_1 = __importDefault(require("../models/Session"));
const User_1 = __importDefault(require("../models/User"));
const auth_middleware_1 = __importDefault(require("../middleware/auth.middleware"));
const router = express_1.default.Router();
// POST /api/sessions – Book a new session with validation
router.post('/', auth_middleware_1.default, [
    (0, express_validator_1.body)('mentorId').isMongoId().withMessage('Valid mentorId is required'),
    (0, express_validator_1.body)('date').isISO8601().toDate().withMessage('Valid date is required'),
    (0, express_validator_1.body)('time').notEmpty().withMessage('Time is required'),
    (0, express_validator_1.body)('topic').notEmpty().withMessage('Topic is required'),
    (0, express_validator_1.body)('notes').optional().isString(),
], (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    var _a;
    const errors = (0, express_validator_1.validationResult)(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    }
    try {
        const { mentorId, date, time, topic, notes } = req.body;
        const menteeId = (_a = req.user) === null || _a === void 0 ? void 0 : _a.userId;
        if (!menteeId) {
            return res.status(401).json({ message: 'Unauthorized: No user info' });
        }
        const mentor = yield User_1.default.findById(mentorId);
        if (!mentor || mentor.role !== 'mentor') {
            return res.status(400).json({ message: 'Invalid mentor selected' });
        }
        const session = yield Session_1.default.create({
            mentor: mentorId,
            mentee: menteeId,
            date,
            time,
            topic,
            notes,
        });
        res.status(201).json({
            message: 'Session booked successfully',
            session,
        });
    }
    catch (error) {
        res.status(500).json({ message: 'Failed to book session', error });
    }
}));
// ✅ UPDATED: GET /api/sessions – Now includes admin access
router.get('/', auth_middleware_1.default, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    var _a, _b;
    try {
        const userId = (_a = req.user) === null || _a === void 0 ? void 0 : _a.userId;
        const userRole = (_b = req.user) === null || _b === void 0 ? void 0 : _b.role;
        if (!userId) {
            return res.status(401).json({ message: 'Unauthorized: No user ID found' });
        }
        let sessions;
        if (userRole === 'admin') {
            sessions = yield Session_1.default.find()
                .populate('mentor', 'name email')
                .populate('mentee', 'name email');
        }
        else if (userRole === 'mentor') {
            sessions = yield Session_1.default.find({ mentor: userId }).populate('mentee', 'name email');
        }
        else if (userRole === 'mentee') {
            sessions = yield Session_1.default.find({ mentee: userId }).populate('mentor', 'name email');
        }
        else {
            return res.status(403).json({ message: 'Forbidden: Unknown role' });
        }
        res.status(200).json({ sessions });
    }
    catch (error) {
        res.status(500).json({ message: 'Failed to retrieve sessions', error });
    }
}));
// GET /api/sessions/:id – Get specific session
router.get('/:id', auth_middleware_1.default, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    var _a;
    try {
        const sessionId = req.params.id;
        const userId = (_a = req.user) === null || _a === void 0 ? void 0 : _a.userId;
        const session = yield Session_1.default.findById(sessionId)
            .populate('mentor', 'name email')
            .populate('mentee', 'name email');
        if (!session) {
            return res.status(404).json({ message: 'Session not found' });
        }
        if (session.mentor._id.toString() !== userId &&
            session.mentee._id.toString() !== userId) {
            return res.status(403).json({ message: 'Access denied: Not your session' });
        }
        res.status(200).json({ session });
    }
    catch (error) {
        res.status(500).json({ message: 'Failed to retrieve session', error });
    }
}));
// PUT /api/sessions/:id – Approve/reject session (mentor only)
router.put('/:id', auth_middleware_1.default, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    var _a, _b;
    try {
        const sessionId = req.params.id;
        const userId = (_a = req.user) === null || _a === void 0 ? void 0 : _a.userId;
        const userRole = (_b = req.user) === null || _b === void 0 ? void 0 : _b.role;
        const { status } = req.body;
        if (userRole !== 'mentor') {
            return res.status(403).json({ message: 'Only mentors can update session status' });
        }
        if (!['approved', 'rejected'].includes(status)) {
            return res.status(400).json({ message: 'Invalid status. Use "approved" or "rejected".' });
        }
        const session = yield Session_1.default.findById(sessionId);
        if (!session) {
            return res.status(404).json({ message: 'Session not found' });
        }
        if (session.mentor.toString() !== userId) {
            return res.status(403).json({ message: 'You can only update your own session' });
        }
        session.status = status;
        yield session.save();
        res.status(200).json({
            message: `Session ${status} successfully`,
            session,
        });
    }
    catch (error) {
        res.status(500).json({ message: 'Failed to update session', error });
    }
}));
exports.default = router;
